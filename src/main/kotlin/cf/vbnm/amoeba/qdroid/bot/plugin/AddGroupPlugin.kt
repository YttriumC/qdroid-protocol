...
class AddGroupPlugin(
    ...
    override fun apply(bot: QBot, event: GroupRequest) {
        ...
        }).filter { it.name.endsWith(event.userId.toString()) }.also {
            bot.setGroupAddRequest(
                event.flag,
                event.subType,
                false,
                "Sorry, 你的加群请求被拒绝了, 原因是: ${it[0].value}"
            ) {
            ...
            bot.setGroupAddRequest(event.flag, event.subType, true) {
                if (coreProperty["${Constant.CAPTCHA_ENABLE_PREFIX}${event.groupId}"] != "") {
                    ...
                    pluginMessageFilter.addInterceptor(object : PluginMessageInterceptor(event.userId) {
                        ...
                        override fun handleMessage(msg: Message, bot: QBot) {
                            try {
                            ...
                            TimeoutJob.addTimeoutJob(scheduler, 5, TimeUnit.MINUTES) {
                                finished()
                            }
                        }
                    })
